...
@interface MLAreaDataManager  ()

@property (nonatomic,strong) FMDatabase *db;

@end

@implementation MLAreaDataManager
static MLAreaDataManager *manager = nil;
+ (MLAreaDataManager *)shareManager{
...
- (void)cityData:(void (^)(NSMutableArray *dataArray))cityData{
    ...
    while ([result next]) {
        NSString *citrName = [result stringForColumn:@"city_name"];
        [resultArray addObject:citrName];
    }
...
//根据关键字搜索城市
-(void)searchCityData:(NSString *)searchObject result:(void (^)(NSMutableArray *))result{

    NSMutableArray *resultArray = [NSMutableArray new];
    FMResultSet *areaResult = [self.db executeQuery:[NSString stringWithFormat:@"SWLECT DISTINCT area_name,city_name,city_number FROM shop_area WHERE area_name LIKE '%@%%';",searchObject]];
